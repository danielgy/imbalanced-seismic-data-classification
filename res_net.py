...
def model(input_tensor, phase):
    ...
    with tf.variable_scope('layer1-shortcut'):
        weights = tf.get_variable("weight", [CONV3_SIZE_row, 
                                             CONV3_SIZE_col, 
                                             NUM_CHANNELS,
                                             CONV3_DEEP],
                                  initializer=tf.truncated_normal_initializer(stddev=0.1))
    ...
    with tf.variable_scope('layer4-global_pooling'):
        fc2_weights = tf.get_variable("weights",
                                      [CONV3_DEEP, OUTPUT_NODE],
                                      initializer=tf.truncated_normal_initializer(stddev=0.1))
        fc2_biases = tf.get_variable("bias", [OUTPUT_NODE], initializer=tf.constant_initializer(0.1))

        globalpool = tf.nn.avg_pool(shortcut2,
                                    ksize=[1, 1, shortcut2.get_shape()[2], 1],
                                    strides=[1, 1,  shortcut2.get_shape()[2], 1], padding='SAME')
