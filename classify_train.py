        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(TRANING_STEPS):
                ...
                if i % 2000 == 0:
                    ...
                    temp_logit = train_logit.reshape([1, -1]) > Th
                    temp_logit = temp_logit.astype("int")
                    train_predict = temp_logit.reshape(-1)
            ...
            temp_logit = test_logit.reshape([1, -1]) > Th
            temp_logit.astype("int")
            test_predict = temp_logit.reshape(-1)
            try:
...
if __name__ == "__main__":
    ...
    for train, valid in kfolds.split(data, label):
        ...
        # model train
        model_train(train_input, valid_input, pos_weight=pw, weight=feature_num)
        end1 = time.time()
        print("{} cross validation time spend is: {}s".format(cv, (end1-st1)))
        print("*" * 75)

    print("Total time spend is: {}s".format((time.time()-starttime)))
